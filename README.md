...
- The submissions table classifies each entry with a **Submitted** badge in a
  "Links Sent" column (otherwise "Not sent"). This is derived from **live**
  publication state — the admin page reads `/published-links` for all
  destinations and marks a submission Submitted only while its links are
